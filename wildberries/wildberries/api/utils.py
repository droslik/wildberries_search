...
def data_handling(request):
    ...
    if len(file) != 0 and article:
        raise serializers.ValidationError('Only one option is possible: enter the article or attach the file')
    if len(file) == 0 and not article:
        raise serializers.ValidationError('Please enter correct article number or attach the file')
    if len(file) > 0 and not article:
...
def file_processing(request):
    ...
    articles = list(int(article[0]) for article in articles_from_memory)
    return articles
